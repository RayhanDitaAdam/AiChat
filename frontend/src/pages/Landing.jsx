            ...
            <div className="min-h-screen bg-[#020617] text-white selection:bg-indigo-500/30 font-sans overflow-x-hidden">
                ...
                <section className="relative z-10 py-20 px-6 overflow-hidden">
                    <div className="max-w-6xl mx-auto">
                        ...
                        >
                            <div className="flex h-14 items-center justify-between px-6 border-b border-white/5 bg-zinc-900/60 rounded-t-[1.8rem]">
                                ...
                                <div className="text-xs uppercase tracking-widest text-zinc-500 font-black">Live interaction Preview</div>
                            ...
                            <div className="p-10 space-y-8 font-medium">
                                <ChatMessage role="user" text="Halo Heart! Ada stok Indomie Goreng? Lokasi raknya dimana ya?" />
                                <ChatMessage role="ai" text="Halo! Saya cek database sebentar... 🍜" />
                                <ChatMessage role="ai" text="Stok Indomie Goreng tersedia sebanyak 142 pcs. Lokasinya ada di Rak B4 (Lorong Sembako) - Rp 3.100 / pcs." isProduct />
                                <ChatMessage role="user" text="Oke, masukkin ke shopping list saya ya. Sama ingetin kalo harganya turun." />
                                <ChatMessage role="ai" text="Siap! Produk telah ditambahkan ke Shopping List Anda. ✅ Saya akan memberi notifikasi jika ada promo menarik terkait item ini." />
                            </div>
                ...
                {/* Audience Section */}
                <section id="retailers" className="relative z-10 py-32 px-6">
                ...
                {/* FAQ Section */}
                <section id="faq" className="relative z-10 py-32 px-6 bg-zinc-950/50">
...
const ChatMessage = ({ role, text, isProduct }) => (
    <div className={`flex gap-6 items-start ${role === 'user' ? 'opacity-80' : ''}`}>
        <div className={`shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center font-black ${role === 'user' ? 'bg-zinc-800 text-zinc-400 border border-white/5' : 'bg-indigo-600 text-white shadow-lg'}`}>
            {role === 'user' ? 'U' : 'H'}
        </div>
        <div className="flex flex-col gap-3 flex-1 min-w-0">
            <div className={`p-6 rounded-[2rem] border ${role === 'user' ? 'bg-zinc-800/50 border-white/5' : 'bg-zinc-950 border-indigo-500/20'}`}>
                <p className="text-zinc-300 leading-relaxed">{text}</p>
            </div>
            {isProduct && (
                <div className="p-6 rounded-[2rem] bg-indigo-600/10 border border-indigo-500/30 w-fit max-w-sm flex items-center gap-4 group">
                    <div className="w-12 h-12 bg-indigo-600 rounded-xl flex items-center justify-center"><ShoppingBasket className="w-6 h-6" /></div>
                    <div>
                        <div className="text-sm font-black italic">Indomie Goreng (SKU: 412)</div>
                        <div className="text-xs font-bold text-indigo-400">Available in Rack B4. Price: Rp 3.100</div>
                    </div>
                </div>
            )}
        </div>
