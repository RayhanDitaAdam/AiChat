...
export class ChatController {
    ...
    async acceptCall(req, res) {
        try {
            const resolvedOwnerId = _optionalChain([req, 'access', _3 => _3.user, 'optionalAccess', _4 => _4.ownerId]) || _optionalChain([req, 'access', _5 => _5.user, 'optionalAccess', _6 => _6.memberOfId]);
    ...
    async declineCall(req, res) {
        try {
            const resolvedOwnerId = _optionalChain([req, 'access', _7 => _7.user, 'optionalAccess', _8 => _8.ownerId]) || _optionalChain([req, 'access', _9 => _9.user, 'optionalAccess', _10 => _10.memberOfId]);
            if (!resolvedOwnerId) return res.status(403).json({ status: 'error', message: 'Forbidden' });
