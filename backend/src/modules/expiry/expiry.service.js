...
export class ExpiryService {
    ...
    async createExpiry(ownerId, date) {
    ...
    async assignProduct(productExpiryId, productId, ownerId, quantity) {
        // Verify ownership
        const expiry = await prisma.productExpiry.findFirst({
        ...
        return prisma.expiryItem.upsert({
            ...
            update: {
                quantity: _nullishCoalesce(quantity, () => ( null))
            },
            create: {
                productExpiryId,
                productId,
    ...
    async removeProduct(productExpiryId, productId, ownerId) {
        // Verify ownership
        const expiry = await prisma.productExpiry.findFirst({
            where: { id: productExpiryId, ownerId }
        });
