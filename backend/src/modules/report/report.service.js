...
export const getSalesAnalytics = async (ownerId, period = 'daily', contributorId) => {
    ...
    while (iterDate <= now) {
        ...
        if (period === 'daily') {
            ...
            results.push({
                date: key,
                total: _optionalChain([aggregated, 'access', _ => _[key], 'optionalAccess', _2 => _2.total]) || 0,
                profit: _optionalChain([aggregated, 'access', _3 => _3[key], 'optionalAccess', _4 => _4.profit]) || 0
            });
            ...
            results.push({
                date: key,
                total: _optionalChain([aggregated, 'access', _5 => _5[key], 'optionalAccess', _6 => _6.total]) || 0,
                profit: _optionalChain([aggregated, 'access', _7 => _7[key], 'optionalAccess', _8 => _8.profit]) || 0
            });
...
export const getComprehensiveReport = async (ownerId, startDate, endDate, contributorId) => {
    ...
    const categoryBreakdown = typedTransactions.reduce((acc, tx) => {
        tx.items.forEach((item) => {
            const cat = _optionalChain([item, 'access', _9 => _9.product, 'optionalAccess', _10 => _10.category]) || 'Uncategorized';
    ...
    const totalProfit = typedTransactions.reduce((sum, tx) => {
        const txProfit = tx.items.reduce((pSum, item) => {
            ...
            const itemCost = (_optionalChain([item, 'access', _11 => _11.product, 'optionalAccess', _12 => _12.purchasePrice]) || 0) * item.quantity;
...
export const getTopSellingProducts = async (ownerId, limit = 5, contributorId) => {
    ...
    return Promise.all(topProducts.map(async (item) => {
        ...
        return {
            name: _optionalChain([product, 'optionalAccess', _13 => _13.name]) || 'Unknown',
            sales: _optionalChain([item, 'access', _14 => _14._sum, 'optionalAccess', _15 => _15.quantity]) || 0,
            revenue: Number(_optionalChain([item, 'access', _16 => _16._sum, 'optionalAccess', _17 => _17.price]) || 0),
        };
    }));
};
