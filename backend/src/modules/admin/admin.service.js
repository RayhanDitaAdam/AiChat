...
export class AdminService {
    ...
    async updateSystemConfig(config
        ...
        if (config.companyName) {
            ...
            const oldName = _optionalChain([currentConfig, 'optionalAccess', _ => _.companyName]) || 'HeartAI';
            const newName = config.companyName;

            if (oldName !== newName) {
                // Update System AI Prompt
                if (_optionalChain([currentConfig, 'optionalAccess', _2 => _2.aiSystemPrompt]) && currentConfig.aiSystemPrompt.includes(oldName)) {
                    config.aiSystemPrompt = (config.aiSystemPrompt || currentConfig.aiSystemPrompt).split(oldName).join(newName);
                }

                if (_optionalChain([currentConfig, 'optionalAccess', _3 => _3.aiGuestSystemPrompt]) && currentConfig.aiGuestSystemPrompt.includes(oldName)) {
