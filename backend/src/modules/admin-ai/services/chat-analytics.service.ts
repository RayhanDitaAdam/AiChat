...
export class ChatAnalyticsService {
    async getGlobalMetrics(ownerId: string, daysBack: number = 7) {
        ...
        const dailyChatsRaw = await prisma.$queryRaw<any[]>`
        ...
        return {
            totalConversations,
            resolutionStats: resolutionStats.reduce((acc: any, curr) => {
                acc[curr.resolution || 'UNKNOWN'] = curr._count;
                return acc;
            }, {}),
            aiAccuracy: parseFloat(aiAccuracy.toFixed(2)),
            topIntents: intentStats.map((stat: any) => ({ name: stat.status, count: stat._count })),
            dailyConversations
        };
    }

    async getUnresolvedSessions(ownerId: string, limit: number = 50, skip: number = 0) {
    ...
    async tagSession(sessionId: string, ownerId: string, resolution: string, accuracyTag: string) {
