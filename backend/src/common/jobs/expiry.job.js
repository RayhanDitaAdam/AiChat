 function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }import cron from 'node-cron';
...
async function checkProductsExpiry() {
    ...
    try {
        ...
        for (const p of expiredToNotify) {
            if (_optionalChain([p, 'access', _ => _.owner, 'access', _2 => _2.user, 'optionalAccess', _3 => _3.email])) {
        ...
        for (const p of expiringSoonToNotify) {
            if (_optionalChain([p, 'access', _4 => _4.owner, 'access', _5 => _5.user, 'optionalAccess', _6 => _6.email])) {
