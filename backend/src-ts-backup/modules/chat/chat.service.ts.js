...
export class ChatService {
  async processChatMessage(input) {
    ...
    const cleanOwnerId =
      ownerId && ownerId !== "null" && ownerId !== "undefined" ? ownerId : null;
    ...
    const isSopQuery = keywords.some((kw) =>
      [
        "sop",
        "policy",
        "aturan",
    ...
    const [owner, systemConfig, user, _staleCleanup, activeCall] =
      await Promise.all([
    ...
    if (
      (activeCall && activeCall.status === "CALL_ACCEPTED") ||
      (metadata && metadata.staffId)
    ) {
    ...
    let regPrompt =
      ...
      `You are ${companyName}, an AI shopping assistant.`;
    ...
    const category =
      keywords.length > 0 ? "PRODUCT_SEARCH" : "GENERAL_CHITCHAT";
    ...
    const [weather, sessionHistory] = await Promise.all([
      userRole === "REG"
        ...
        : Promise.resolve({ temperature: 0, condition: "NONE" }),
      currentSessionId
    ...
    if (
      ...
      userLat &&
      userLng &&
      WeatherService.isProactiveFruitWeather(weather) &&
      keywords.length > 0 &&
      !isRejection(cleanMsg)
    ) {
      const ownerService = new OwnerService();
      const nearbyRes = await ownerService.findNearbyStores(
        userLat,
        userLng,
        5,
      );
      if (nearbyRes.status === "success" && nearbyRes.stores.length > 0) {
        ...
        nearbyStoresContext =
          ...
          nearbyRes.stores
            .map(
              (s) =>
                `- ${s.name} (Distance: ${s.distance.toFixed(1)}km, Domain: ${s.domain})`,
            )
            .join("\n");
      }
    }

    const medicalContext = user?.medicalRecord
      ? `USER MEDICAL NOTES/ALLERGIES: ${user.medicalRecord}\nCRITICAL: DO NOT recommend products that conflict with these medical notes or allergies.`
      : "";
    ...
    const [products, sops, stats] = await Promise.all([
      ...
      keywords.length > 0 ||
      cleanMsg.includes("cari") ||
      cleanMsg.includes("ada")
      ...
      isManagement || isSopQuery
        ? sopService.getSopsByOwner(cleanOwnerId)
        : Promise.resolve([]),
      // Fetch Stats if user is STAFF/OWNER
      isManagement
        ? statService.getOwnerStats(cleanOwnerId)
        : Promise.resolve(null),
    ]);
    ...
    if (contextProducts.length === 0 && sessionHistory.length > 0) {
      const lastAiWithProducts = sessionHistory.find(
        (h) => h.role === "ai" && h.metadata?.products?.length > 0,
      );
    ...
    const managementContext =
      isManagement || isSopQuery
        ? `MGMT_STATS: ${JSON.stringify(stats?.stats || [])}\ncompanyDocs: ${JSON.stringify(formattedSops)}`
        : "NONE";
    ...
    const pipelineResult = await chatPipelineService.process(
      ...
      (chunk) => {
        if (targetSocketId) {
          io.to(targetSocketId).emit("ai_chunk", {
            sessionId: currentSessionId,
            chunk,
          });
        }
    ...
    if (pipelineResult.metadata?.products) {
      contextProducts = [
        ...contextProducts,
        ...pipelineResult.metadata.products,
      ];
    }
    ...
    if (status === "NOT_FOUND" && cleanOwnerId) {
      const query =
        keywords.length > 0 ? keywords.join(" ") : message.substring(0, 50);
      prisma.missingRequest
        .upsert({
    ...
    if (cleanUserId) {
      const remindMatches = cleanMessage.matchAll(
        /\[REMIND:\s*([^\]|]+)\|?([^\]]*)\]/g,
      );
    ...
    if (autoAddedProductIds.length > 0 && cleanUserId) {
      ...
      for (const pid of autoAddedProductIds) {
        try {
          await shoppingListService.addItem(cleanUserId, pid);
        } catch (err) {}
      }
    }

    // Filter products
    const filteredProducts = contextProducts.filter(
      (p) =>
  ...
  async getSessions(userId, ownerId, excludeStaffChats = false) {
    ...
    if (excludeStaffChats) {
      const filteredSessions = sessions.filter((session) => {
        const hasStaffStatus = session.chats?.some(
          (msg) =>
            msg.status &&
            [
              "CALL_PENDING",
  ...
  async requestStaff(userId, ownerId, latitude, longitude, targetStaffId) {
    ...
    const isActive =
      latestInteraction &&
      ["CALL_PENDING", "CALL_ACCEPTED"].includes(latestInteraction.status);
    ...
    await prisma.chatHistory.create({
      data: {
        ...
        owner_id: ownerId,
        message:
          "Sistem: User memerlukan bantuan staff. (System: User requires staff assistance.)",
        role: "ai",
        // @ts-ignore
        status: "CALL_PENDING",
    ...
    io.to(`store_${ownerId}`).emit("chat_requested", {
      userId,
      ownerId,
      targetStaffId,
      timestamp: new Date(),
  ...
  async acceptCall(userId, ownerId) {
    ...
    await prisma.chatHistory.updateMany({
      ...
      data: {
        ...
        message:
          "Sistem: Staff telah bergabung ke chat. (System: Staff joined the chat.)",
      },
    });
  ...
  async declineCall(userId, ownerId) {
    ...
    await prisma.chatHistory.update({
      ...
      data: {
        ...
        message:
          "Sistem: Staff menolak permintaan chat. (System: Staff declined the chat request.)",
      },
    });
