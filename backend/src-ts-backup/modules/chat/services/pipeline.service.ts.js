  ...
  async process(
    ...
    if (confidence > 0.7) {
      if (["order_status", "refund_request", "complaint"].includes(intent)) {
        ...
        intent === "action" ||
        message.toLowerCase().includes("cari") ||
        message.toLowerCase().includes("ada")
      ) {
        // Check if it's a product search
        console.log("⚡ Pipeline: Fast Product Search...");
        const productResult = await this.productService.search(
          message,
          ownerId,
        );
        if (productResult) {
    ...
    if (!finalResponse || finalResponse.trim() === "") {
      ...
      if (["faq", "greeting", "unknown"].includes(intent)) {
        const contextDoc = await this.faqService.search(
          message,
          ownerPrefix,
          5,
          true,
