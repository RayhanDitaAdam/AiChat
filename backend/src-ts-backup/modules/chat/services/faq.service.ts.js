...
export class FaqService {
  ...
  async initChroma() {
    try {
      ...
      console.warn(
        "ChromaDB not available. Falling back to Prisma DB for FAQ matching.",
      );
          ...
          for (let i = 0; i < distanceArray.length; i++) {
            ...
            if (dist < 0.2 || (returnDetailed && dist < 0.5)) {
              ...
              if (returnDetailed) {
                ...
                let answer = meta?.answer || null;
                let finalMetadata = {};

                if (faqId) {
                  const fullFaq = await prisma.faqItem.findUnique({
                    where: { id: faqId },
                  });
                  if (
                    fullFaq &&
                    fullFaq.productIds &&
                    fullFaq.productIds.length > 0
                  ) {
                    const products = await prisma.product.findMany({
                      where: {
                        id: { in: fullFaq.productIds },
                        owner_id: ownerId,
                      },
                    });
                    const productText = products
