  ...
  async chat(req, res) {
    try {
      ...
      if (req.user.role === "OWNER" && req.user.ownerId) {
        const [stats, products, forecasting, vacancies, sops] =
          await Promise.all([
        ...
        console.log(
          `[AI-Chat] Fetching SOPs for owner ${req.user.ownerId}... Found: ${sops.length}`,
        );
      ...
      const response = await AIService.generateManagementResponse(
        message,
        context,
        req.user.role,
      );
