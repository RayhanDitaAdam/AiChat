...
export const scrapeTokopedia = async (req, res) => {
  try {
    ...
    const outputPath = path.join(
      __dirname,
      "../../../../scraper/tokopedia_products.json",
    );
    ...
    child.on("close", async (code) => {
      ...
      if (code !== 0) {
        res
          ...
          .json({
            status: "error",
            message: "Scraper failed to execute completely",
          });
        return;
      }
      ...
      try {
        if (!fs.existsSync(outputPath)) {
          res
            ...
            .json({
              status: "error",
              message: "Scraper finished but output file not found",
            });
          return;
        }
        ...
        if (!Array.isArray(products) || products.length === 0) {
          res
            .status(404)
            .json({ status: "error", message: "No products found or scraped" });
          return;
        }

        console.log(
          `[Scraper] Successfully scraped ${products.length} products. Inserting to database...`,
        );
        ...
        console.error(
          "[Scraper] Error parsing output or inserting to DB:",
          err,
        );
        res
          .status(500)
          .json({ status: "error", message: "Failed to process scraped data" });
      }
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Failed to initiate scrape" });
  }
};
