...
export class BuildingController {
  async createSubLocation(req, res) {
    try {
      ...
      const result = await buildingService.createSubLocation(
        req.user.ownerId,
        req.body,
      );
  ...
  async getSubLocations(req, res) {
    try {
      const ownerId =
        req.params.ownerId || req.user?.ownerId || req.user?.memberOfId;
      if (!ownerId) {
        return res
          .status(400)
          .json({ status: "error", message: "Owner ID missing" });
      }
  ...
  async updateSubLocation(req, res) {
    try {
      ...
      const { id } = req.params;
      const result = await buildingService.updateSubLocation(
        id,
        req.user.ownerId,
        req.body,
      );
  ...
  async deleteSubLocation(req, res) {
    try {
      ...
      const { id } = req.params;
      const result = await buildingService.deleteSubLocation(
        id,
        req.user.ownerId,
      );
      return res.json(result);
