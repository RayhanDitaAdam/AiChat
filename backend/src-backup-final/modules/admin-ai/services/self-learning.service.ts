...
export class SelfLearningService {
     ...
     */
    async discoverNewKnowledge(ownerId: string, daysBack: number = 1) {
...
`;
    ...
    async getSuggestions(ownerId: string, status: string = 'PENDING') {
    ...
    async approveSuggestion(suggestionId: string, ownerId: string, finalQuestion: string, finalAnswer: string, categoryId?: string) {
    ...
    async rejectSuggestion(suggestionId: string, ownerId: string) {
