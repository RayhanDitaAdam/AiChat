...
export class SopController {
    ...
    async uploadSop(req: Request, res: Response) {
        try {
    ...
    async getSops(req: Request, res: Response) {
        try {
            const ownerId = req.user?.ownerId || req.user?.memberOfId;
            if (!ownerId) {
                return res.status(403).json({ status: 'error', message: 'Not an owner or staff' });
    ...
    async deleteSop(req: Request, res: Response) {
        try {
            const ownerId = req.user?.ownerId || req.user?.memberOfId;
    ...
    async updateSopText(req: Request, res: Response) {
        try {
            const ownerId = req.user?.ownerId || req.user?.memberOfId;
            const id = req.params.id as string;
