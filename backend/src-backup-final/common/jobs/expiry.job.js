...
export const initExpiryJob = () => {
  ...
  console.log(
    "[ExpiryJob] Product expiry job initialized (runs every 30 minutes).",
  );
};
  ...
  try {
    ...
    for (const p of expiredToNotify) {
      if (p.owner.user?.email) {
        console.log(
          `[ExpiryJob] Sending EXPIRED notification for ${p.name} to ${p.owner.user.email}`,
        );
    ...
    for (const p of expiringSoonToNotify) {
      if (p.owner.user?.email) {
        console.log(
          `[ExpiryJob] Sending EXPIRING_SOON notification for ${p.name} to ${p.owner.user.email}`,
        );
        await EmailService.sendExpiryNotification(
          p.owner.user.email,
          p.owner.user.name || "Owner",
    ...
    if (expiredToNotify.length > 0 || expiringSoonToNotify.length > 0) {
      console.log(
        `[ExpiryJob] Notified ${expiredToNotify.length} expired and ${expiringSoonToNotify.length} expiring soon products.`,
      );
    }
  } catch (error) {
