...
export class VacancyService {
    ...
    async updateApplicationStatus(ownerId, applicationId, status) {
        ...
        if ((status === 'ACCEPTED' || status === 'REJECTED') && application.user?.email) {
            try {
                await EmailService.sendApplicationStatusEmail(application.user.email, application.user.name, application.vacancy.owner.name, status, application.vacancy.title);
            }
            catch (error) {
