...
AI Response:`;
    ...
    static async generateGuestResponseStream(message, context, language = 'id', systemPrompt, config, onChunk, history = []) {
        ...
        try {
            ...
            const companyName = guestConfig?.companyName || (await prisma.systemConfig.findUnique({ where: { id: 'global' } }))?.companyName || 'HeartAI';
...
AI Response:`;
    ...
    static async generateManagementResponse(message, context, userRole, config) {
        ...
        if (!model)
            return "AI service unavailable.";
        try {
            const companyName = config?.companyName || (await prisma.systemConfig.findUnique({ where: { id: 'global' } }))?.companyName || 'HeartAI';
